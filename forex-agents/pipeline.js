...
// Runs one evaluation cycle for a single forex pair: Research -> Discussion
// -> Portfolio Management -> General Manager -> Executor. Unlike the
// crypto team, this isn't event-driven off a volume surge — every pair is
// always liquid, so this simply runs once per newly-closed candle per pair
// instead, called in a loop from index.js across every configured
// instrument each tick.
async function runCycle(state, instrument) {
  const cooldown = riskGuard.checkCooldown(state, instrument);
  if (!cooldown.ok) {
    return null; // already in a position or on cooldown for this pair — nothing to evaluate
  }

  const candles = await oandaClient.getCandles(instrument, config.CANDLE_GRANULARITY, config.CANDLE_COUNT);
  ...
  if (latestCandleTime === state.lastSignalCandleTimes[instrument]) {
    return null; // already evaluated this candle for this pair, nothing new yet
  }
  ...
  const freshPrice = await oandaClient.getPrice(instrument);
  const freeBalance = await balanceCache.getFreeBalance(state);
  const positionSizing = await sizePosition(instrument, freshPrice, freeBalance);
